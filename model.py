...
def inference(images, batch_size, n_classes):
    ...
    with tf.variable_scope('conv1') as scope:
        weights = tf.Variable(tf.truncated_normal(shape=[3, 3, 3, 100], stddev=1.0, dtype=tf.float32),
                              name='weights', dtype=tf.float32)

        biases = tf.Variable(tf.constant(value=0.1, dtype=tf.float32, shape=[100]),
                             name='biases', dtype=tf.float32)

        conv = tf.nn.conv2d(images, weights, strides=[1, 1, 1, 1], padding='SAME')
        pre_activation1 = tf.nn.bias_add(conv, biases)
    ...
    with tf.variable_scope('conv4') as scope:
        ...
        biases = tf.Variable(tf.constant(value=0.1, dtype=tf.float32, shape=[10]),
                                 name='biases', dtype=tf.float32)
    ...
    with tf.variable_scope('conv5') as scope:
        ...
        biases = tf.Variable(tf.constant(value=0.1, dtype=tf.float32, shape=[7]),
                                 name='biases', dtype=tf.float32)

        conv = tf.nn.conv2d(conv4, weights, strides=[1, 1, 1, 1], padding='SAME')
        pre_activation5 = tf.nn.bias_add(conv, biases)
        conv5 = tf.nn.relu(pre_activation5, name='conv5')
    # 池化层2
    # 3x3最大池化，步长strides为2，池化后执行lrn()操作，
    # pool2 and norm2
    with tf.variable_scope('pooling2_lrn') as scope:
        norm2 = tf.nn.lrn(conv5, depth_radius=4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name='norm2')
        pool2 = tf.nn.max_pool(norm2, ksize=[1, 3, 3, 1], strides=[1, 1, 1, 1], padding='SAME', name='pooling2')
    # 全连接层3
    # n_Neuron个神经元，将之前pool层的输出reshape成一行，激活函数relu()
    with tf.variable_scope('local3') as scope:
        reshape = tf.reshape(pool2, shape=[batch_size, -1])
        dim = reshape.get_shape()[1].value
        weights = tf.Variable(tf.truncated_normal(shape=[dim, n_Neuron], stddev=0.005, dtype=tf.float32),
                              name='weights', dtype=tf.float32)

        biases = tf.Variable(tf.constant(value=0.1, dtype=tf.float32, shape=[n_Neuron]),
                             name='biases', dtype=tf.float32)

        local3 = tf.nn.relu(tf.matmul(reshape, weights) + biases, name=scope.name)
    # 全连接层4
    # n_Neuron个神经元，激活函数relu()
    with tf.variable_scope('local4') as scope:
        weights = tf.Variable(tf.truncated_normal(shape=[n_Neuron, n_Neuron], stddev=0.005, dtype=tf.float32),
                              name='weights', dtype=tf.float32)
    ...
    with tf.variable_scope('softmax_linear') as scope:
        weights = tf.Variable(tf.truncated_normal(shape=[n_Neuron, n_classes], stddev=0.005, dtype=tf.float32),
